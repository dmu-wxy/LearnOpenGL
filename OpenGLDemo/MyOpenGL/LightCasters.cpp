...
LightCasters::LightCasters(unsigned int screen_width, unsigned int screen_height) : IDemo(screen_width, screen_height), cubeVAO(-1), lightCubeVAO(-1), VBO(-1), pLightingShader(nullptr), pLightCubeShader(nullptr) {}
...
void LightCasters::init() {
    ...
    // ------------------------------------
    pLightingShader = new Shader("2.2.basic_lighting.vs", "2.5.basic_lighting.fs");
...
void LightCasters::draw(Camera camera, glm::vec3 lightPos) {
    ...
    pLightingShader->setVec3("light.direction", -0.2f, -1.0f, -0.3f);
